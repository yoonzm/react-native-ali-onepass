...
/**定义页面present方向*/
typedef NS_ENUM(NSUInteger, NTESCMPresentationDirection){
    NTESCMPresentationDirectionBottom = 0,  //底部  present默认效果
    NTESCMPresentationDirectionRight,       //右边  导航栏效果
    NTESCMPresentationDirectionTop,         //上面
    NTESCMPresentationDirectionLeft,        //左边
};
...
@property (nonatomic, assign) NTESCMPresentationDirection presentType;
/**4、授权界面背景图片*/
...
/**45、自定义窗口宽-缩放系数(屏幕宽乘以系数) 默认是0.8*/
@property (nonatomic,assign) CGFloat scaleW;

/**46、自定义窗口高-缩放系数(屏幕高乘以系数) 默认是0.5*/
@property (nonatomic,assign) CGFloat scaleH;

/**47、返回按钮x偏移(负数左边偏移,正数右边偏移) 只有隐藏导航栏时有效 默认frame为 (20,居中,20 * 系数scaleW,20 * scaleW) */
...
/**48、此属性支持半弹框方式与authWindow不同（此方式为UIPresentationController）设置后自动隐藏切换按钮*/
